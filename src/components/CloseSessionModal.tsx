...
import { sessionService, Session, CloseSessionPayload } from '../services/sessionService';
...
const CloseSessionModal: React.FC<CloseSessionModalProps> = ({ isOpen, onClose, onSuccess, sessionToClose }) => {
    ...
    if (!isOpen || !sessionToClose) return null;

    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4 transition-opacity duration-300">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-md transform transition-all duration-300 text-gray-800 dark:text-gray-200">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-xl font-semibold">Fechar Caixa (Sessão #{sessionToClose.id})</h2>
                    <button onClick={handleCloseAndReset} disabled={isLoading} className="text-gray-400 hover:text-gray-700 dark:hover:text-gray-300" aria-label="Fechar">
                ...
                <div className='text-sm space-y-1 border dark:border-gray-600 rounded p-3 mb-4 bg-gray-50 dark:bg-gray-700/50'>
                     ...
                     <p className='flex justify-between'><span>Valor Inicial:</span> <span className='font-medium'>{formatCurrencyModal(Number(sessionToClose.valor_abertura))}</span></p>
                     <p className='flex justify-between'><span>Saldo Calculado (Sistema):</span> <span className='font-medium text-blue-600 dark:text-blue-400'>{formatCurrencyModal(Number(sessionToClose.valor_fechamento_calculado))}</span></p>
                 </div>

                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label htmlFor="valor-informado" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Valor Contado em Caixa *</label>
                        <NumericFormat
                            id="valor-informado"
                    ...
                    <div>
                        <label htmlFor="observacao" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Observação de Fechamento (Opcional)</label>
                        <textarea id="observacao" rows={2} value={observacao} onChange={(e) => setObservacao(e.target.value)} disabled={isLoading}
                                  className="w-full border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 rounded-md px-3 py-2 text-sm focus:ring-indigo-500 focus:border-indigo-500 disabled:bg-gray-100 dark:disabled:bg-gray-800"
                                  placeholder='Ex: Diferença no caixa, sangria não registrada, etc.' />
                    </div>

                    <div className="flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-3 gap-3 pt-4 border-t dark:border-gray-600">
                        <button type="button" onClick={handleCloseAndReset} disabled={isLoading} className="w-full sm:w-auto justify-center px-4 py-2 border border-gray-300 dark:border-gray-500 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50">Cancelar</button>
