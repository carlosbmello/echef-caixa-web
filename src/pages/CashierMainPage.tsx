...
// import { paymentService, Payment } from '../services/paymentService'; // Removido, não é mais usado
...
// [CORREÇÃO] Interface local para representar um pagamento na UI antes de finalizar a transação.
// Ela é mais simples que a interface 'Payment' completa do banco de dados.
interface UIPayment {
  id: number;
  valor: string;
  data_hora: string;
...
const CashierMainPage: React.FC = () => {
    ...
    const [groupPaymentsList, setGroupPaymentsList] = useState<UIPayment[]>([]); // [CORREÇÃO] Usa a nova interface local
    ...
    const handleRegisterPayment = async (e: React.FormEvent) => { e.preventDefault(); if (!selectedComandas.length || !openSession || !paymentValueCents) return; const fId = parseInt(selectedPaymentMethodId); const valInReais = paymentValueCents / 100; if (isNaN(fId) || fId <= 0 || valInReais <= 0) { setComandaError("Verifique a forma de pagamento e o valor."); return; } if (valInReais > groupSaldoDevedor + 0.01) { setComandaError(`Valor do pagamento ${formatCurrency(valInReais)} é maior que o saldo devedor ${formatCurrency(groupSaldoDevedor)}.`); return; } setIsProcessingPayment(true); setComandaError(null); try { const newPayment: UIPayment = { id: Date.now(), valor: valInReais.toString(), data_hora: new Date().toISOString(), nome_forma_pagamento: paymentMethods.find(m => m.id === fId)?.nome || 'N/A', detalhes: paymentDetails.trim() || null }; setGroupPaymentsList(prevList => [...prevList, newPayment]); toast.success(`Pagamento de ${formatCurrency(valInReais)} adicionado à lista.`); setSelectedPaymentMethodId(''); setPaymentValueCents(null); setPaymentDetails(''); setPaymentInputKey(Date.now()); } catch (err: any) { setComandaError(err.message); toast.error(err.message || "Erro ao adicionar pagamento."); } finally { setIsProcessingPayment(false); } };
    const handleFinalizarTransacao = async () => { if (!selectedComandas.length || !openSession) { toast.warn("Nenhuma comanda selecionada para finalizar."); return; } if (groupSaldoDevedor > 0.01) { toast.error(`Ainda há um saldo pendente de ${formatCurrency(groupSaldoDevedor)}.`); return; } if (!window.confirm(`Tem certeza que deseja finalizar e fechar ${selectedComandas.length} comanda(s)?`)) { return; } setIsProcessingPayment(true); setError(null); setComandaError(null); try { const payload: FinalizarTransacaoPayload = { comandaIds: selectedComandas.map(c => c.id), taxa_servico: groupTaxaServico, acrescimos: groupAcrescimosCents / 100, descontos: groupDescontosCents / 100, pagamentos: groupPaymentsList.map(p => { const methodId = paymentMethods.find(pm => pm.nome === p.nome_forma_pagamento)?.id; return { forma_pagamento_id: methodId || 0, valor: Number(p.valor), nome_forma_pagamento: p.nome_forma_pagamento }; }) }; if (payload.pagamentos.some(p => p.forma_pagamento_id === 0)) { throw new Error("Não foi possível identificar a forma de pagamento de um dos lançamentos."); } const response = await transacaoService.finalizar(payload); toast.success(response.message || "Comanda(s) finalizada(s) com sucesso!"); setSelectedComandas([]); handleVoltarParaMonitor(); } catch (err: any) { toast.error(err.message || "Erro ao finalizar a transação."); } finally { setIsProcessingPayment(false); } };
    ...
    const handlePaymentValueChange = (v: NumberFormatValues) => { setPaymentValueCents(v.floatValue ? Math.round(v.floatValue * 100) : null); if(comandaError) setComandaError(null); };
    ...
    const handlePrintConferencia = async () => { if (!selectedComandas.length || isPrinting) return; setIsPrinting(true); setComandaError(null); const PONTO_ID_CAIXA = 3; try { const jobData = { cabecalho: { linha1: "NEVERLAND BAR", linha2: "Sua casa de espetaculos" }, comandas: selectedComandas.map(comanda => ({ numero: comanda.numero, clienteNome: comanda.cliente_nome, itens: comandaItems.filter(item => item.numero_comanda === comanda.numero).map(item => ({ quantidade: `${formatQuantity(Number(item.quantidade || 0))}x`, nome: item.produto_nome, valor: formatCurrency(Number(item.quantidade || 0) * Number(item.preco_unitario_momento || 0)) })) })), resumoTransacao: { consumo: [{ descricao: "Total Consumo", valor: formatCurrency(groupTotalConsumo) }], taxaServico: { descricao: "(+) Taxa de Serviço (10%)", valor: formatCurrency(groupTaxaServico) }, acrescimos: { descricao: "(+) Acréscimos", valor: formatCurrency(groupAcrescimosCents / 100) }, descontos: { descricao: "(-) Descontos", valor: formatCurrency(groupDescontosCents / 100) }, totalConta: { descricao: "Total da Conta", valor: formatCurrency(groupTotalAPagar) } } }; await printService.imprimirPorPonto(PONTO_ID_CAIXA, jobData, 'clienteConta'); toast.success("Impressão de conferência enviada!"); } catch (err: any) { setComandaError(err.message); toast.error(err.message); } finally { setIsPrinting(false); } };
    const handleShowOpenModal=()=>{setError(null);setShowOpenModal(true);}; const handleShowCloseModal=()=>{setError(null);setShowCloseModal(true);}; const handleShowMovementModal=()=>{setError(null);setShowMovementModal(true);}; const handleOpenSuccess=()=>{setShowOpenModal(false);fetchInitialData();fetchOpenComandas();}; const handleCloseSuccess=()=>{setShowCloseModal(false);fetchInitialData();fetchOpenComandas();}; const handleMovementSuccess=()=>{}; const handleLogout=()=>{logout();}; const handleIniciarFechamento=()=>{setComandaError(null);setViewMode('fechamento');};
